...
if isnumeric(sj),
    ...
    subjectdata.csvdir              = sprintf('%s/P%02d/CSV',  path, sj);
    if ~exist(subjectdata.csvdir, 'dir'), mkdir(sprintf('%s/P%02d/',  path, sj), 'CSV'); end
end
...
if ~isnumeric(sj),
    ...
    subjectdata.eyedir      = sprintf('%s/GrandAverage/Eye',  path);
    subjectdata.figsdir     = sprintf('%s/Figures', path);
    subjectdata.csvdir      = sprintf('%s/CSV', path);
    
    % subject lists
    subjectdata.all         = [2:65];
    % remove those subjects that did not complete the task and/or did not come for MRI
    subjectdata.all(ismember(subjectdata.all, [11 31 37])) = [];

    % remove one subject with fewer than 100 trials in each session lots of head movement
    ...
    switch lower(sj)
        ...
        case 12
            ...
            subjectdata.session(1).rec(5).blocks        = 5:8; % EOGH and EOGV switched...
            subjectdata.session(1).restingstate         = 1;
