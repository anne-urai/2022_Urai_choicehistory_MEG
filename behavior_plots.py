...
sns.lineplot(data=df_subj_melt, x='x', y='value', units='subj_idx', hue='repetition',
              ...
              ax=ax, style=df_subj_melt.group, 
...
corrfunc(df_subj_session_melt[1], df_subj_session_melt[2], 
         ...
         df_subj_melt.loc[df_subj_melt.variable == 'repeat', 'group'].values,
         ax)
...
# group split
df_kernels['group'] = 0
df_kernels.loc[df_kernels.repetition < 0.5, 'group'] = -1
df_kernels.loc[df_kernels.repetition > 0.5, 'group'] = 1
...
stats = df_kernels.groupby(['group', 'Lags'])['resp_kernel'].apply(ttest)
...
fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(4,2), 
                       sharex=True)
sns.lineplot(data=df_kernels[df_kernels.group != 0].reset_index(), 
             x='Lags', y='resp_kernel', hue='group',
             legend=False, palette='PuOr', hue_norm=(-1.5, 1.5), #palette='ch:s=.25,rot=-.25',
             ax=ax[0], style='group',
             dashes=False, markers=['o', '^'], 
             err_style='bars', zorder=200)
sns.lineplot(data=df_kernels[df_kernels.group != 0].reset_index(), 
             x='Lags', y='stim_kernel', hue='group',
             legend=False, palette='PuOr', hue_norm=(-1.5, 1.5), # palette='ch:s=.25,rot=-.25',
             ax=ax[1], style='group',
             dashes=False, markers=['o', '^'],
...
corrfunc(df_kernels.resp_kernel, df_kernels.repetition, 
         df_kernels.repetition, df_kernels.group,
         ax[0])
...
sns.scatterplot(data=df_kernels,  ax=ax,
                        ...
                        style='true_repeaters', hue='group', palette='PuOr', hue_norm=(-0.5, 1.5))
...
sns.scatterplot(x=df_kernels.resp_kernel, y=df_kernels.stim_kernel, 
            hue=df_kernels.repetition, style=df_kernels.group,
            legend=False, palette='PuOr', hue_norm=(0.4,0.6),#palette='ch:s=.25,rot=-.25', 
            markers=marker_set,
            ax=ax)
...
df_new = pd.merge(df_kernels[['subj_idx', 'repetition', 'group', 'resp_kernel']], 
...
print(list(df_new.loc[df_new['true_repeaters'] == True, 'subj_idx'])) # 12 true repeaters
...
plt.close('all')
fig, ax = plt.subplots(nrows=1, ncols=1, figsize=[1.5, 2])
df_new['x'] = df_new.group + df_new.repetition + 0.3*np.random.normal(size=df_new.repetition.shape)
df_new.loc[df_new.repetition == 0.5, 'x'] = 0.5
df_new.signif = 1 * df_new.signif

sns.scatterplot(data=df_new, ax=ax,
                        y='repetition', x='x', size='signif',
                        style='group', hue='repetition', 
...
#%% extra: quick plot of RT distributions, only trials for HDDMnn
plt.close('all')
...
# how many trials pp?
trialcounts = data.groupby(['subj_idx'])['response'].count().reset_index()
trialcounts.describe()
