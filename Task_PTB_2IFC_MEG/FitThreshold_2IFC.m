function [datapoints1, datapoints2, datapoints3, fit1, fit2, fit3] = FitThreshold_2IFC(dataset, fitting, bootstrapping, BAC)%% fit a psychometric function
...
% reshape
datapoints1.all.difficulty  = reshape(setup.coherence', 1, setup.nblocks*setup.ntrials);
...
fit1.threshold = fit1.x(dsearchn(fit1.y', fit1.perflevel));
disp(sprintf('%02d PERCENT THRESHOLD FOR THIS SUBJECT IS', fit1.perflevel*100));
disp(fit1.threshold);

suptitle(sprintf('Subject %d, threshold %.3f', setup.participant, fit1.threshold));

%save the fig
set(gcf,'PaperOrientation','landscape');
set(gcf,'PaperUnits','normalized');
set(gcf,'PaperPosition', [0 0 1 1]);
saveas(gcf, sprintf('Data/PsychFuncFit_P%d.png', setup.participant), 'png');
