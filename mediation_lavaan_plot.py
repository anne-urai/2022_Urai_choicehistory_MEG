...
rep = data.groupby(['subj_idx'])['repeat', 'group'].mean().reset_index()
...
for trials in ['all', 'error', 'correct']:
    ...
    for vsidx, varsets in enumerate([['indirect_gamma',  'indirect_alpha', 'indirect_betalat', 'direct'],
        ...
        for gridx, groups in enumerate(['all', 'alternators', 'repeaters']):
            ...
            # merge these different ones, create new repo
            df = pd.merge(df_lavaan2, rep, on='subj_idx')
            ...
            for vidx, v in enumerate(varsets):
                ...
                # do stats on the posterior distribution
                pval = ttest_against_zero['p-val'].item()
                print('%s trials, %s %s: t(%d) = %.3f, p = %.4f'%(trials, v, groups,
                                                     ttest_against_zero['dof'].item(),
                                                 ttest_against_zero['T'].item(),
                                                 ttest_against_zero['p-val'].item()))
                if ttest_against_zero['p-val'].item() < 0.0001:
                    print(np.format_float_scientific(ttest_against_zero['p-val'].item(), precision = 2, exp_digits=3))
                ...
                if df[v].mean() > 0:
                    ...
                    star_y = ttest_against_zero['CI95%'].item()[0] / 10000 * 1.8
                #star_y = -ylims[vsidx][vidx] / 1.3
                ...
                if gridx == 2:
                    ttest2 = pg.ttest(df_keep.loc[df_keep.group == 1, v], 
                                      df_keep.loc[df_keep.group == -1, v], paired=False)
                    pval = ttest2['p-val'].item()
                    txt = "p = {:.4f}".format(pval)
                    
                    print(ttest2[['T', 'dof', 'p-val']])
    ...
    for vsidx, varsets in enumerate([['indirect_gamma_stim',  'indirect_gamma_delay']]):
        ...
        for gridx, groups in enumerate(['all', 'alternators', 'repeaters']):
            ...
            for vidx, v in enumerate(varsets):
                ...
                print('WM %s %s: t(%d) = %.3f, p = %.4f'%(v, groups, 
                                                     ttest_against_zero['dof'].item(),
                                                 ttest_against_zero['T'].item(),
                                                 ttest_against_zero['p-val'].item()))
