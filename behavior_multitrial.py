...
# map each person's repetition behavior
df_subj = df.groupby(['subj_idx'])['repeat'].mean().reset_index()

# define groups, exclude P39
df_subj['repeaters'] = np.nan
df_subj.loc[df_subj.repeat < 0.5, 'repeaters'] = 0
df_subj.loc[df_subj.repeat > 0.5, 'repeaters'] = 1

df_subj = df_subj.rename(columns={'repeat':'repetition'})
df = pd.merge(df, df_subj, on='subj_idx')
...
for correct_only_all in [False]:
    for last_trial in ['any', 'error', 'correct']:
        for first_trial in ['any', 'error', 'correct']:
            ...
            df_sequences = df.groupby(['repeaters', 'repetition',
            ...
            df_sequences_avg = df_sequences_toflip.groupby(['repeaters', 'repetition',
                                                     'subj_idx',
                                                     'match_final',
            ...
            for vidx, v in enumerate(plot_vars):
                for gridx, whichgroup in enumerate([False, True]):
                    ...
                    sns.lineplot(data=df_sequences_avg.loc[df_sequences_avg.repeaters == whichgroup],
                                  marker='o', ax=ax[vidx, gridx + 2], 
                                  **kwargs)
                    
                    # do an ANOVA
                    anov = pg.rm_anova(data = df_sequences_avg.loc[(df_sequences_avg.repeaters == whichgroup) 
                ...
                if v == 'criterion':
                    ...
                    for gr, x in df_sequences_avg.groupby(['repeaters', 'repetition', 
                                                             'subj_idx']):
                        print(x[['criterion', v]])
                    
                    similarity_index = df_sequences_avg.groupby(['repeaters', 'repetition', 
                    ...
                    ttest_random = pg.ttest(similarity_index['r'], 0)
                    ttest_rep = pg.ttest(similarity_index[similarity_index.repeaters == True]['r'], 0)
                    ttest_alt = pg.ttest(similarity_index[similarity_index.repeaters == False]['r'], 0)

                    # now also test this as a fixed effect
                    group_fixed = df_sequences_avg.groupby(['nr_reps','match_final']).mean()
                    rep_fixed = df_sequences_avg[df_sequences_avg.repeaters == True].groupby(['nr_reps','match_final']).mean()
                    alt_fixed = df_sequences_avg[df_sequences_avg.repeaters == False].groupby(['nr_reps','match_final']).mean()
                    ...
                    # print the stats in the title
                    stats_str = 'random t(%02d) = %.2f, p = %.3f\n'%(ttest_random['dof'][0], 
                                                                             ttest_random['T'][0], ttest_random['p-val'][0]) + \
                                'random alt p = %.3f, random rep p = %.3f\n'%(ttest_alt['p-val'][0], ttest_rep['p-val'][0]) + \
...
df_jasp = pd.pivot_table(df_sequences_saveall, index=['subj_idx', 'repeaters'],
                         values='gamma_ips23_stimwin', columns=['nr_reps', 'match_final'])
df_jasp = pd.DataFrame(df_jasp.to_records())

print(df_jasp)
df_jasp.to_csv(os.path.join(mypath, 'CSV', 'sequences_akaishi_jasp_criterion.csv'))
...
for plot_var in ['criterion', 'gamma_ips23_stimwin', 
    ...
    for gridx, gr in enumerate([False, True]):
        sns.lineplot(data=df_sequences_saveall.loc[(df_sequences_saveall.repeaters == gr) & \
        ...
        data_subset_2nd = df_sequences_saveall.loc[(df_sequences_saveall.repeaters == gr) & \
        ...
        # do an ANOVA
        data_subset_anov = df_sequences_saveall.loc[(df_sequences_saveall.repeaters == gr) & \
                                                       (df_sequences_saveall.nr_reps > 0) & \
                                                   (df_sequences_saveall.first_trial == 'any') & \
                                                       (df_sequences_saveall.last_trial == 'any') & \
                                                           (df_sequences_saveall.correct_only_all == False)]
    ...
    df_sequences_saveall.groupby(['repeaters'])[['subj_idx']].nunique()
...
for gridx, gr in enumerate([False, True]):
    ...
    data_subset = df_sequences_saveall.loc[(df_sequences_saveall.repeaters == gr) & \
...
# grab the mail data
data_subset = df_seq[['subj_idx', 'repeaters', 'repetition',
                           'nr_reps', 'match_final',
...
similarity_index = data_subset.groupby(['repeaters', 'repetition', 
                                         'subj_idx']).apply(lambda x: 
                                                            stats.pearsonr(x['criterion'], 
...
ttest_rep = sp.stats.ttest_1samp(similarity_index[similarity_index.repeaters == True]['r'], 0)
ttest_alt = sp.stats.ttest_1samp(similarity_index[similarity_index.repeaters == False]['r'], 0)
